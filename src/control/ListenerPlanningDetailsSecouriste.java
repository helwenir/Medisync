...
public class ListenerPlanningDetailsSecouriste implements ListChangeListener<Agenda.Appointment> {

    private VueDetailEvent VueDetailEvent;
    private App a;
    private Controller c;

    public ListenerPlanningDetailsSecouriste(App a,Controller c){
        this.a=a;
        this.c=c;
    }
        ...
        while (appointmentsList.next()) {
            if (appointmentsList.wasAdded()) {
                for (Agenda.Appointment app : appointmentsList.getAddedSubList()) {
                    this.VueDetailEvent=new VueDetailEvent(app.getSummary() + " :", app.getLocation(),  app.getStartLocalDateTime().format(formatter) + " - " + app.getEndLocalDateTime().format(formatter), app.getDescription(), null,false,this.c);
